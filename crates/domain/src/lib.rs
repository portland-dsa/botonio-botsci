...
//! newtypes ([`ids`]), the [`DryRun`] flag every write path carries, the managed
//! [`Role`] tiers, the raw [`MigsStatus`] membership-status value shared across
//! sources, and the computed [`MembershipStatus`] with its mapping to a role.
//!
